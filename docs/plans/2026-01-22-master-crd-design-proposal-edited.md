...
1. **Push-based Sentinel discovery**: Valkey explicitly references Sentinel (self-documenting)
1. **Per-instance Sentinel config**: Different SLAs for different instances
1. **ValkeyNode abstraction**: Generic, configuration-driven pod management
1. **AZ-aware distribution**: Cost optimization for cross-AZ traffic
1. **Progressive enhancement**: Natural upgrade paths from simple to complex
...
1. **Kubernetes-native**: Follow established Kubernetes conventions for consistency
1. **Production focus**: Prioritize cluster and replication modes for production workloads
1. **Upgrade paths**: Enable seamless progression from standalone → replicated → HA → sharded

### Design Principles

**Kubernetes-native conventions:**
...
**Spec describes relationships, not roles**
...
ValkeyPool would provide a means of easily scaling a number of independent instances that share the same configuration. This is useful in larger deployments where Valkey is used as the Sidekiq datastore.
