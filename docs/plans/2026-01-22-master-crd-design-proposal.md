# Valkey Operator CRD Architecture - Master Design Proposal

**Date:** 2026-01-22
**Status:** Master Proposal (Synthesized from Proposals 1, 2, and 3)
**Authors:** Claude Code (synthesis of community proposals)
...
3. [CRD Architecture Overview](#crd-architecture-overview)
4. [ValkeyNode CRD (Internal)](#valkeynode-crd-internal)
5. [Valkey CRD (Standalone/Replicated)](#valkey-crd-standalonereplicated)
6. [ValkeyPool CRD (Client-Side Sharding)](#valkeypool-crd-client-side-sharding)
7. [ValkeyCluster CRD (Server-Side Sharding)](#valkeycluster-crd-server-side-sharding)
8. [Sentinel CRD (HA Failover)](#sentinel-crd-ha-failover)
9. [Common Configuration Patterns](#common-configuration-patterns)
10. [Key Design Decisions](#key-design-decisions)
11. [Design Rationale and Rejected Alternatives](#design-rationale-and-rejected-alternatives)
...
This master proposal synthesizes three independent design proposals for the Valkey Kubernetes Operator, taking the best elements from each to create a comprehensive, production-ready CRD architecture.

### Five-CRD Architecture

| CRD | Purpose | User-Facing | Source |
|-----|---------|-------------|--------|
| **ValkeyNode** | Single-pod abstraction | ❌ No (operator-managed) | Proposals 2 & 3 |
| **Valkey** | Standalone/replicated instance | ✅ Yes | All proposals |
| **ValkeyPool** | Multiple independent instances | ✅ Yes | All proposals |
| **ValkeyCluster** | Sharded cluster mode | ✅ Yes | All proposals |
| **Sentinel** | HA monitoring and failover | ✅ Yes | All proposals |

### Key Innovations

1. **Kubernetes-native semantics**: `replicas` = total pods (not replicas excluding primary)
2. **Role derivation**: No `role` field in spec; derived from `replicaOf` to avoid failover conflicts
3. **Push-based Sentinel discovery**: Valkey explicitly references Sentinel (self-documenting)
4. **Per-instance Sentinel config**: Different SLAs for different instances
5. **ValkeyNode abstraction**: Generic, configuration-driven pod management
6. **AZ-aware distribution**: Cost optimization for cross-AZ traffic
7. **Progressive enhancement**: Natural upgrade paths from simple to complex
...
**From Proposal 3 - Kubernetes-native conventions:**
- `replicas` = **total number of pods** (consistent with Deployment/StatefulSet)
- Role derived from configuration, not specified explicitly
- Controlled scaling (±1 changes only for safety)

**From Proposal 1 - Progressive enhancement:**
- Users start simple and add complexity as needed
- No forced migration between CRD types
- Clear upgrade paths within each CRD

**From Proposal 2 - Explicit over implicit:**
- Failover modes explicitly declared
- No external dependencies (TLS via Secret references)
- Self-documenting specifications

**From All Proposals - Separation of concerns:**
- ValkeyNode handles infrastructure
- Parent controllers handle topology
- Sentinel handles failover (when configured)

**From Proposal 1 - Nested sub-problems:**
- Related fields grouped (`persistence.rdb`, `tls.clusterBus`)
- Better organization and extensibility

**From Proposal 3 - Configuration-driven behavior:**
- ValkeyNode behavior determined by config fields
...
- Creates Deployment or StatefulSet (replicas=1)
- Creates Service (stable DNS per pod)
- Creates PVC (if persistence enabled)
- Configures Valkey/Sentinel based on spec
...
spec:
  ...
  valkeyConfig:
    ...
    # - replicaOf: "hostname:port" → This node is a REPLICA
    replicaOf: ""
...
1. Failovers change roles dynamically
2. Spec represents desired state
...
**Role is derived from `replicaOf` field:**
- `replicaOf: ""` (empty) → Node acts as a **primary**
- `replicaOf: "host:port"` → Node acts as a **replica**

**Status tracks observed reality:**
- `observedRole` and `observedReplicaOf` queried from `INFO replication`
- During failovers, status may temporarily differ from spec
...
5. Valkey controller updates ValkeyNode specs to match reality
6. **Spec follows reality**
...
4. ValkeyNode controllers run `REPLICAOF` commands
5. ValkeyNode controllers update status
6. **Spec leads, reality follows**

### Configuration-Driven Behavior

ValkeyNode behavior determined by config fields, not parent CRD type:

| Configuration | Resulting Behavior |
|---------------|-------------------|
| `cluster.enabled: true, slots: ["0-5460"]` | Valkey Cluster primary |
| `cluster.enabled: true, replicaOf: "host:port"` | Valkey Cluster replica |
...
**Cost optimization:** Reduces cross-AZ data transfer costs while maintaining HA.
...
```python
# Python client-side sharding example
startup_nodes = [
...
func (v *ValkeyCluster) ValidateUpdate(old *ValkeyCluster) error {
  ...
  // Replicas can only change by ±1
  replicaDiff := abs(v.Spec.Replicas - old.Spec.Replicas)
  if replicaDiff > 1 {
...
spec:
  ...
  # Default Sentinel config (fallback if Valkey doesn't specify)
  defaultConfig:
    downAfterMilliseconds: "30000"
    failoverTimeout: "180000"
    parallelSyncs: "1"
...
status:
  ...
  # Monitored Valkey instances (populated by Valkey controllers)
  monitoredInstances:
    - name: prod-cache
      namespace: default
...
### 1. Replicas Semantic - Kubernetes-Native
...
**Decision:** ValkeyNode spec has no `role` field. Role is derived from `replicaOf`.

**Implementation:**
- `replicaOf: ""` → primary
- `replicaOf: "host:port"` → replica
- Status tracks `observedRole` (queried from `INFO replication`)

**Rationale:**
- Failovers change roles dynamically
- Spec represents desired state
- Having `role: primary` would conflict with failover decisions
- Avoids spec/reality conflicts

**Failover semantics:**
- **Sentinel-managed**: Spec follows reality
- **Operator-managed**: Spec leads, reality follows

### 3. ValkeyNode - Generic Pod Abstraction

**Decision:** ValkeyNode is operator-managed, supports Valkey and Sentinel via `type` field.
...
- Service per pod
- PVC managed separately
- Maximum scheduling control

### 4. Nested Sub-Problems
...
### 7. Per-Instance Sentinel Configuration
...
### 8. Singleton Deployments/StatefulSets
...
### 9. Operator Creates PVCs Separately
...
**Decision:** Each ValkeyNode gets its own Service.
...
### 15. ValkeyCluster Breaking Changes

**Decision:** Change `replicas` semantic pre-0.1.0.

**Rationale:**
- Pre-release is perfect time
- Consistency across CRDs
- Better UX long-term

**Migration:** `new_replicas = old_replicas + 1`

### 16. Controlled Scaling

**Decision:** Count fields can only change by ±1 per update.

**Rationale:**
- Safe, gradual scaling
- Prevents accidents
- Easier rollback
...
**Chosen:** Role derived from `replicaOf`

**Rejected:** Explicit `role: primary|replica` field

**Rationale:**
- Failovers change roles
- Spec/reality conflicts avoided
- Works with both operator and Sentinel failover
...
**Chosen:** Service per pod

**Rejected:** Shared headless service
...
- Per-shard availability guarantees
- Safe node drains
...
### 9. Explicit Failover Trigger

- ValkeyFailover CRD for controlled failovers
- Pre-maintenance failovers
...
This master proposal synthesizes:
- **Proposal 1** (2026-01-19): docs/plans/2026-01-19-crd-design.md
- **Proposal 2** (2026-01-20): docs/plans/2026-01-20-crd-architecture-design.md
- **Proposal 3** (2026-01-22): docs/plans/2026-01-22-crd-architecture-proposal-3.md
...
## Appendix: Total Pod Count Examples
...
- `shards: 5, template.replicas: 2` → **10 pods** (5 shards × 2 pods)
